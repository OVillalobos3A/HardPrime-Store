...
if (isset($_GET['action'])) {
    ...
    switch ($_GET['action']) {
                    ...
                    if ($recu->generarCodigo(5)) {
                        if ($recu->updateRecu()) {
                            if ($recu->obtenerCorreo()) {
                                ...
                                $mail->Body = 'El código de recuperación es: ' . $recu->getCodigoAutn() . '.';
                                if ($mail->Send()) {
                                    $result['status'] = 1;
        ...
        case 'recuContra':
            $_POST = $recu->validateForm($_POST);
            if ($recu->setCodigoAutn($_POST['codigo'])) {
                if ($recu->setUsuario($_POST['usuario2'])) {
                    if ($recu->checkCode() == true) {                        
                        if ($recu->getCodigoAutn() == $_POST['codigo']) {
                            if ($_POST['pass1'] == $_POST['pass2']) {
                                if ($_POST['pass1'] != $_POST['usuario2']) {
                                if ($recu->setContraseña($_POST['pass1'])) {
                                    if ($recu->recuContra()) {
                                        $result['status'] = 1;
