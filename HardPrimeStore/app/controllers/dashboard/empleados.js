...
const API_EMPLEADOS = '../../app/api/dashboard/emp.php?action=';
  ...
  dataset.map(function (row) {
      ...
      content += `
          <tr>
              ...
              <td><b>${row.telefono}</b></td>
              <td><b>${row.correo}</b></td>              
              <td><b>${row.genero}</b></td>
              <td><b>${row.estado}</b></td>
              <td><img src="../../resources/img/productos/${row.imagen}" class="materialboxed" height="100"></td>              
              <td>
              <a href="#modal_registro" onclick="openUpdateDialog(${row.id_empleado})" class="btn-floating btn waves-effect light-blue darken-4 modal-trigger"><i class="material-icons" title="Editar registro">create</i></a>
              <a href="#" onclick="openDeleteDialog(${row.id_empleado})" class="btn-floating btn waves-effect red"><i class="material-icons" title="Eliminar registro">delete</i></a>
              </td>
          </tr>
      `;
  });
